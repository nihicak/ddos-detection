...
blocked_ips = set()
lock = threading.Lock()
CONNECTION_THRESHOLD = 14
TIME_WINDOW = 30
SHORT_WINDOW_THRESHOLD = 10
SHORT_WINDOW = 5
LOG_WINDOW = 5
SERVER_IP = ""
SERVER_NAME = ""
...
def monitor_connections():
    sniff(filter="tcp port 9944", prn=log_connection)
...
def log_connection(packet):
    ...
    if ip == SERVER_IP or ip in blocked_ips or ip in get_validator_ips():
        return
    if flag != 'S':
        return
    ...
    with lock:
        ...
        if check_spam(ip):
            print(f"DETECTED SPAMMING on Port 9944 from {ip}")
            deny_ip(ip)
            connection_log[ip] = []

            try:
                alert_slack(f"{SERVER_IP} [{SERVER_NAME}] - DETECTED SPAMMING on Port 9944 from {ip}")
            except Exception as e:
                print(f"Error alert to slack: {e}")

        elif len(connection_log[ip]) >= CONNECTION_THRESHOLD:
            print(f"DETECTED DDOS on Port 9944 from {ip}")
            deny_ip(ip)
            connection_log[ip] = []

            try:
                alert_slack(f"{SERVER_IP} [{SERVER_NAME}] - DETECTED DDOS on Port 9944 from {ip}")
            except Exception as e:
                print(f"Error alert to slack: {e}")
...
def deny_ip(ip):
    ufw_command = f"sudo ufw insert 1 deny from {ip}" # for process
    # ufw_command = f"sudo ufw route insert 1 deny from {ip}" # docker
